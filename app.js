...
async function exchangeCode(code) {
  ...
  // Prepare Basic Auth header with support for special characters
  const credentials = idp.clientId + ':' + idp.clientSecret;
  // Convert to UTF-8 before base64 encoding to handle special characters
  const utf8Credentials = unescape(encodeURIComponent(credentials));
  const basicAuthHeader = 'Basic ' + btoa(utf8Credentials);
...
async function fetchUserInfo(idp, tokens) {
  ...
  console.log('Request — Endpoint: ', idp.userinfoEndpoint);
  console.log('Request — Headers:  ', requestHeaders);

  try {
    const resp = await fetch(idp.userinfoEndpoint, { headers: requestHeaders });
    ...
    let info;
    try { info = JSON.parse(rawText); } catch(e) { info = { error: 'unparseable_response', error_description: rawText }; }

    if (!resp.ok || info.error) {
      setStep('userinfo', 'error', 'Error ' + resp.status + ': ' + (info.error_description || rawText.slice(0, 200)));
      return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.modal-backdrop').forEach(m => {
    m.addEventListener('click', e => {
      // Do nothing — require explicit Save or Cancel to close modals
    });
  });
});
...
function checkUrlCallback() {
  ...
  try {
    ...
    setStep('state',     'done', 'State & PKCE restored from session storage');
    setStep('authorize', 'done', 'IDP redirect completed — code received');
    document.getElementById('manualCodeCard').style.display  = '';
    document.getElementById('flowRequestCard').style.display = '';
